...
When the issue has been triaged, an an appropriate label (per https://golang.org/wiki/HandlingIssues.md) to mark it as such.
...
If you have access (see https://golang.org/wiki/GerritAccess.md) to run the TryBots and you see a CL with plausible (and non-malicious) code, kick off the TryBots. (We've never seen malicious code trying to escape our TryBot sandboxes, but that's why it's not automatic yet. Please alert us if you see something.)
